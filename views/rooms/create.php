<h4>Thêm phòng</h4>
<form method="post" action="<?= h(base_url('index.php?r=rooms/store')) ?>" class="row g-3">
  <div class="col-md-4">
    <label class="form-label">Mã phòng</label>
    <input class="form-control" name="room_code" value="<?= h($old['room_code'] ?? '') ?>">
    <?php if (!empty($errors['room_code'])): ?><div class="text-danger small"><?= h($errors['room_code']) ?></div><?php endif; ?>
  </div>
  <div class="col-md-4">
    <label class="form-label">Sức chứa</label>
    <input class="form-control" name="capacity" type="number" min="1" value="<?= h($old['capacity'] ?? '4') ?>">
